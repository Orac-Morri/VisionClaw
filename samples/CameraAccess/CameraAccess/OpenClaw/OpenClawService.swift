...
final class OpenClawService: ObservableObject {
    ...
    //
    // Ported from OpenVision, which stores a whole `ws://host:port` string. VisionClaw keeps the
    // host and port separately (Secrets/SettingsManager), so the URL is assembled here instead.
    //
    // `openClawHost` must stay a BARE, SINGLE-LABEL hostname (e.g. "orac"). The app's
    // NSAppTransportSecurity sets NSAllowsLocalNetworking, which makes iOS ignore
    // NSAllowsArbitraryLoads — so cleartext ws:// is permitted only to "local" destinations.
    // Unqualified hostnames qualify; fully-qualified names (orac.tailXXXX.ts.net) and the
    // 100.x CGNAT tailnet IPs do NOT, and fail with NSURLErrorDomain -1022. With Tailscale's
    // MagicDNS the short name resolves both on the LAN and remotely, so one value covers both.

    private var gatewayURL: URL? {
        let host = SettingsManager.shared.openClawHost
            .replacingOccurrences(of: "http://", with: "")
            .replacingOccurrences(of: "https://", with: "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        guard !host.isEmpty else { return nil }
        return URL(string: "ws://\(host):\(SettingsManager.shared.openClawPort)")
    }

    private var authToken: String {
        SettingsManager.shared.openClawGatewayToken
    }
    ...
    private let networkQueue = DispatchQueue(label: "visionclaw.network")
    ...
    private func sendHandshake() async throws {
        ...
        var params: [String: Any] = [
            ...
            "client": [
                "id": "cli",
                // This is the name the gateway shows in `openclaw devices list`. It MUST differ
                // from OpenVision's: both apps pair as separate devices (separate Keychain
                // identities), and two entries both labelled "OpenVision" make it impossible to
                // tell which is which when revoking one.
    ...
    private static var sessionKey = "visionclaw-\(UUID().uuidString.prefix(8))"
